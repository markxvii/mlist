...
from mlist.apis.v1.auth import auth_required, generate_token
from mlist.apis.v1.errors import api_abort, ValidationError
from mlist.apis.v1.schemas import user_schema, item_schema, items_schema
...
class IndexAPI(MethodView):
    def get(self):
        return jsonify({
            "api_version": "1.0",
            "api_base_url": "http://mlist.com/api/v1",
            "current_user_url": "http://mlist.com/api/v1/user",
            "authentication_url": "http://mlist.com/api/v1/token",
            "item_url": "http://mlist.com/api/v1/items/{item_id }",
            "current_user_items_url": "http://mlist.com/api/v1/user/items{?page,per_page}",
            "current_user_active_items_url": "http://mlist.com/api/v1/user/items/active{?page,per_page}",
            "current_user_completed_items_url": "http://mlist.com/api/v1/user/items/completed{?page,per_page}",
        })
...
class ItemsAPI(MethodView):
    ...
    def post(self):
        """Create new item"""
        item = Item(body=get_item_body(), author=g.current_author)
        db.session.add(item)
        db.session.commit()
        response = jsonify(item_schema(item))
        response.status_code = 201 #201表示已创建
